...
def bundleAdjustment(x, mean_chessboard_pose, chessboard_calibrator_undistorted, debug=False):
...
def calibrate_extrinsics(initial_guess, robot_poses, chessboard_calibrator, w, h, sqs):
        ...
        for it, uimg in enumerate(chessboard_calibrator.undistored_images):
            rv = chessboard_calibrator_undistorted.consumeImage(uimg)
            if rv is not None:
        ...
        for i, robot_pose in enumerate(chessboard_calibrator_undistorted.robot_poses):
            ...
            row = np.append(np.array([chessboard_pose[0,3], chessboard_pose[1,3], chessboard_pose[2,3]]), np.asarray(quatFromMatrix(chessboard_pose)))
            row_sum += row
        ...
        mean_chessboard_bose = fromPoseToMatrix(row_sum)
        print("MEasurements")
        print(measurements)
        print("MEan")
        print(row_sum)
        ...
        bundleAdjustment(T_init_guess, mean_chessboard_bose, chessboard_calibrator_undistorted)


        x0 = initial_guess
        translation_bounds = [-1, 1, -1, 1, -1, 1]
...
if __name__ == "__main__":
    ...
    ap.add_argument("--image_folder", required=False, default="/home/lar/ros/uc1_ws/src/camera_calibration/right_arm_2206", help="Image Folder")
    ap.add_argument("--image_extension", default='png', type=str)
    ap.add_argument("--chessboard_size", default='7x5', type=str)
    ap.add_argument("--chessboard_square_size", default=0.0175, type=float)
    ...
    calibrate_extrinsics(initial_guess, robot_poses_filt, chessboard_calibrator, w, h, sqs)